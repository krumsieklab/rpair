...
SUBROUTINE phuhnet (delta, lam2, nobs, nvars, x, y, jd, pf, pf2, dfmax, &
& pmax, nlam, flmin, ulam, eps, isd, maxit, nalam, b0, beta, ibeta, &
& nbeta, alam, npass, ncp, pp, jerr)
! --------------------------------------------------
      IMPLICIT NONE
    ...
    ! - - - local declarations - - -
      ...
      !DOUBLE PRECISION :: xh(ncp, nvars)
      ALLOCATE (xh(1:ncp,1:nvars), STAT=ierr)
      jerr = jerr + ierr
      !nobs = ncp
      xh =  x(pp(:,1),:) - x(pp(:,2),:)
      !------------------------
...
! - - - allocate variables - - -
      ...
      pf = Max (0.0D0, pf)
      pf2 = Max (0.0D0, pf2)
      CALL standard (ncp, nvars, xh, ju, isd, xmean, xnorm, maj)
      CALL hsvmlassoNETpath (delta, lam2, maj, ncp, nvars, xh, y, ju, &
...
! - - - organize beta afterward - - -
      DO l = 1, nalam
         ...
         IF (isd == 1) THEN
            ...
            END DO
         END IF
         b0 (l) = b0 (l) - 0.0D0 !dot_product (beta(1:nk, l), xmean(ibeta(1:nk)))
      END DO
      DEALLOCATE (ju, xmean, xnorm, maj, xh)
      RETURN
END SUBROUTINE phuhnet
...
! --------------------------------------------------
SUBROUTINE chkvarss (nobs, nvars, x, ju)
! --------------------------------------------------
      IMPLICIT NONE
    ! - - - arg types - - -
      INTEGER :: nobs
...
! --------------------------------------------------
SUBROUTINE psqhnet (lam2, nobs, nvars, x, y, jd, pf, pf2, dfmax, pmax, &
& nlam, flmin, ulam, eps, isd, maxit, nalam, b0, beta, ibeta, nbeta, &
& alam, npass, ncp, pp, jerr)
! --------------------------------------------------
      IMPLICIT NONE
...
! - - - allocate variables - - -
      ...
      pf2 = Max (0.0D0, pf2)
      CALL standard (ncp, nvars, xh, ju, isd, xmean, xnorm, maj)
      CALL sqsvmlassoNETpath (lam2, maj, ncp, nvars, xh, y, ju, pf, pf2, &
